...
(function ($, window, document, _, moment, undefined) {
  ...
  var Easycal = {
    ...
    _attachEventHandlers: function () {
      var self = this;
      self.$elem.find('table.' + classes.timeGridTable + ' .' + classes.timeSlot)
        .on('click', function (ev) {
          if ($(ev.target).closest('.' + classes.eventContainer).length ||
            $(ev.target).hasClass(classes.eventContainer)) {
            if (typeof self.options.eventClick === 'function') {
              var $eventContainer =
                (($(ev.target).closest('.' + classes.eventContainer).length) ?
                  $(ev.target).closest('.' + classes.eventContainer) :
                  $(ev.target));
    ...
    _detachEventHandlers: function () {
      this.$elem.find('table.' + classes.timeGridTable + ' .' + classes.timeSlot)
        .off();
    },
    ...
    _mapEventsByDate: function () {
      ...
      for (var i = 0; i < 7; i++) {
        ...
        var filteredEvents = _.filter(events, function (event) {
          if ((moment(event.start, format.dateLong).format(format.dateShort)) ===
            dateStr) {
            return true;
          }
        });
    ...
    _showEvents: function () {
      ...
      var eventDateMap = this._mapEventsByDate(),
        $col = null,
        $slots = null,
        $slot = null,
        schedule = null,
          ...
          _.each($slots, function (slot, i) {
            ...
            if (scheduleForSlot.length > 1) {
              ...
              $slot
                ...
                .addClass(classes.eventContainer)
                .attr('data-event-id', scheduleForSlot[0].id);

              var slotStartTime =
                moment(colDate + ' ' + slotTime, format.dateLong);
              var eventStartTime =
                moment(scheduleForSlot[0].start, format.dateLong);
              ...
              if ($slot.hasClass(classes.minorSlot)) {
                var parentSlotTime =
                  $slot.parent('.' + classes.timeSlot).attr('data-time');
                var momParentSlotTime =
                  moment(colDate + ' ' + parentSlotTime, format.dateLong);
    ...
    isSpanMultipleSlots: function (schedule) {
      if (schedule.length === 1) {
        var startTime = moment(schedule[0].start, format.dateLong)
          .add(this.options.slotDuration, 'm');
    ...
    renderSlotHTML: function (scheduleList) {
      ...
      if (scheduleList.length > 1) {
        ...
        var startTime =
          moment(schedule.start, format.dateLong).format(format.timeShort);
        var endTime =
          moment(schedule.end, format.dateLong).format(format.timeShort);
        html += '' +
          '<div style="padding-top: 4px;">' +
          '<div class="' + classes.timeRange + '">' + startTime + ' - ' +
          endTime + '</div>' +
          '<div class="' + classes.eventTitle + '">' + schedule.title +
          '</div>' +
          '</div>';
      }
      return html;
    ...
    getDaySchedule: function (dayEvents) {
      var date =
        moment(dayEvents[0].start, format.dateLong).format(format.dateShort);
    ...
    renderHTML: function () {
      return '<table border="0" cellpadding="0" cellspacing="0" class="easycal">' +
        '<thead>' +
        '<tr>' +
        '<td>' + (this.renderHeadHTML()) + '</td>' +
        '</tr>' +
        '</thead>' +
        '<tbody>' +
        '<tr>' +
        '<td class="' + classes.timeGridContainer + '">' +
        (this._renderTimeGridHTML()) + '</td>' +
        '</tr>' +
        '</tobdy' +
        '</table>';
    },
    ...
    renderHeadHTML: function () {
      ...
      var html =
        '<table border="0" cellspacing="0" cellpadding="0" class="ec-head-table"><tbody><tr>';
      for (var i = 0; i < 8; i++) {
        var cellContent = '';
        if (i !== 0) {
    ...
    _renderTimeGridHTML: function () {
      ...
      var html =
        '<table border="0" cellspacing="0" cellpadding="0" class="ec-time-grid-table"><tbody><tr>';
      ...
      for (var i = 0; i < 8; i++) {
        ...
        for (; time.isBefore(maxTime);) {
          if (i === 0) {
            ...
            html += '<div class="table-cell ' + classes.timeLabel + '">' +
              cellContent + '</div>';
          } else {
            timeTag = time.format(format.timeLong);
            html += '<div class="table-cell ' + classes.timeSlot +
              '" data-time="' + timeTag + '">';
    ...
    _getMinorSlotForCell: function (momTime) {
      ...
      if (this.options.timeGranularity < this.options.slotDuration) {
        for (var i = 0; i < (this.options.slotDuration / this.options.timeGranularity); i++) {
          html += '<div class="' + classes.minorSlot + '" data-time="' +
            time.format(format.timeLong) + '"></div>';
          time.add(this.options.timeGranularity, 'm');
        }
      }
      return html;
    ...
    _inflateMinorSlots: function () {
      var granularityLevel =
        this.options.slotDuration / this.options.timeGranularity;
      var minorSlotHeight = self.slotHeight / granularityLevel;
      this.$elem
        .find('table.' + classes.timeGridTable + ' td .' + classes.minorSlot)
        .css({
  ...
  $.fn.easycal = function (options) {
    ...
    if (typeof options === 'undefined' || typeof options === 'object') {
      ...
      this.each(function () {
        ...
        if (Easycal.isPrototypeOf(instance) &&
